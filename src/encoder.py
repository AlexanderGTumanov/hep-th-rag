...
def build_vocab(chunks, min_freq = 5, max_vocab = None, vocab_path = "../encoder/vocab.jsonl", overwrite = False):
    counter = Counter()
    for chunk in chunks:
...
def load_vocab(vocab_path = "../encoder/vocab.jsonl"):
...
class ArxivChunkDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        meta = {"chunk_id": record.get("chunk_id"), "doc_id": record.get("doc_id"), "section_id": record.get("section_id")}
...
class ChunkEncoder(nn.Module):
    def __init__(
        ...
        if use_positional_encoding:
            self.pos_emb = nn.Embedding(max_len + (1 if pooling_type == "cls" else 0), d_model)
        else:
            self.pos_emb = None
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.use_positional_encoding:
            pos = torch.arange(seq_len, device = input_ids.device).unsqueeze(0).expand(B, seq_len)
...
def top_lexical_chunks(prompt, lexical_candidates = 1000, method = "bm25", corpus_dir = "../data/corpus", embeddings_subdir = "embeddings"):
    ...
    key_tokens = ("tokens-v1", chunks_path, chunks_mtime, ids_path, ids_mtime, tuple(sorted(placeholders)))
    if key_tokens not in _INDEX_CACHE:
        tokenized_corpus = []
        texts = []
        for cid in chunk_ids:
            ...
            body = re.sub(r"[\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE63\uFF0D]", "-", body)
            body = latinize(body.lower())
            tokens = [t for t in _WORD_RE.findall(body or "") if t not in placeholders]
    ...
    if method == "tfidf":
        ...
        key_lex = ("bm25-chunk-v1", key_tokens)
        if key_lex not in _INDEX_CACHE:
            bm25 = BM25Okapi(tokenized_corpus)
            _INDEX_CACHE[key_lex] = bm25
        bm25 = _INDEX_CACHE[key_lex]
        qbody = (prompt or "").strip()
        qbody = re.sub(r"[\u2010\u2011\u2012\u2013\u2014\u2015\u2212\uFE63\uFF0D]", "-", qbody)
        qbody = latinize(qbody.lower())
        qtokens = [t for t in _WORD_RE.findall(qbody or "") if t not in placeholders]
        lex_scores = np.array(bm25.get_scores(qtokens))
    else:
...
def dense_rerank(
    ...
    raise ValueError("method must be one of: 'dense', 'lexical', 'pin_top', 'score_fusion', 'rrf'.")
...
def aggregate_to_papers(chunk_scores, pool_level = "section", top_k_chunks = 1, top_k_sections = 1, corpus_dir = "../data/corpus"):
    ...
    for doc_id, entry in per_doc.items():
        ...
        for sec_name, sec in entry["sections"].items():
            ...
            if top_k_chunks >= 0:
                kk = min(top_k_chunks, len(scores))
            else:
                kk = max(0, len(scores) + top_k_chunks + 1)
            if kk == 0:
                continue
        ...
        if pool_level == "paper":
            ...
            if not section_scores:
                continue
            if top_k_sections >= 0:
                kk = min(top_k_sections, len(section_scores))
            else:
                kk = max(0, len(section_scores) + top_k_sections + 1)
            if kk == 0:
                continue
...
def fuse_paper_rankings(*rankings, k_fusion = 10):
    rank_maps = []
    meta = {}
    ...
    for doc_id in all_doc_ids:
        ...
        for rm in rank_maps:
            r = rm.get(doc_id)
            if r is not None:
                score += 1.0 / (k_fusion + r)

        base = meta.get(doc_id, {})
        fused.append({
            "score": float(score),
            "doc_id": doc_id,
            "title": base.get("title"),
            "authors": base.get("authors") or [],
            "n_chunks": base.get("n_chunks", 0),
